  ...
  if (rangePriceSlider.length) {
    rangePriceSlider.forEach((range) => {
      ...
      if (startValue && endValue) {
        ...
        var setValues = setValues2;
        range.noUiSlider.on("update", function(values, handle) {
          inputs[handle].value = values[handle];
        });
        startValue.addEventListener("change", setValues2);
        endValue.addEventListener("change", setValues2);
      }
    });
  }
}
document.querySelector("[data-fls-range-price]") ? window.addEventListener("load", rangeInit) : null;
(() => {
...
function zoomInit() {
  ...
  function setStyles(cloneImage, zoomImage, mousePos, zoomValue) {
    const imageSize = {
      ...
      height: zoomImage.offsetHeight * zoomValue
    };
...
document.querySelector("[data-fls-zoom]") ? window.addEventListener("load", zoomInit) : null;
...
function toggleLockSliderClass(sliderEl) {
  const nextBtn = sliderEl.parentElement.querySelector(".swiper-button-next");
  const myBlock = sliderEl.parentElement.querySelector("[data-swiper-lock]");
  if (!myBlock) return;
  if (nextBtn && nextBtn.classList.contains("swiper-button-lock")) {
...
function initSliders() {
  ...
  if (document.querySelector(".popular-cateories__slider")) {
    const proposalsSliders = document.querySelectorAll(".popular-cateories__slider");
    proposalsSliders.forEach((slider, index) => {
      const parentSlider = slider.parentElement;
      const swiperNextBtn = parentSlider.querySelector(".swiper-button-next");
      const swiperPrevBtn = parentSlider.querySelector(".swiper-button-prev");
  ...
  if (document.querySelector(".latestnews__slider")) {
    const proposalsSliders = document.querySelectorAll(".latestnews__slider");
    proposalsSliders.forEach((slider, index) => {
      const parentSlider = slider.parentElement;
      const swiperNextBtn = parentSlider.querySelector(".swiper-button-next");
      const swiperPrevBtn = parentSlider.querySelector(".swiper-button-prev");
